...
from salle_activite.models import Activity, Door, Salle
...
class IndexView(PermissionRequiredMixin, SingleTableMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_dettes = (
            AbonnementClient.objects.aggregate(total_reste=Sum("reste"))["total_reste"]
            or 0
        )
        context["total_dettes"] = total_dettes

        chiffre_affaire = (
            Paiement.objects.aggregate(total_chiffre=Sum("amount"))["total_chiffre"]
            or 0
        )
        ...
        depenses_1 = (
            Remuneration.objects.aggregate(depenses_1=Sum("amount"))["depenses_1"] or 0
        )
        depenses_2 = (
            RemunerationProf.objects.aggregate(depenses_2=Sum("amount"))["depenses_2"]
            or 0
        )
        ...
        queryset = (
            ...
            .values("creneau__activity__salle__name")
    ...
    def get_queryset(self):
        ...
        remunerationProf = RemunerationProf.objects.filter(
            date_creation=today
        ).order_by()
...
class PorteUpdateView(UpdateView):
    model = Door
    template_name = "configuration/snippets/_ajout_ports_form.html"
    fields = ["ip_adress", "salle", "username", "password"]
