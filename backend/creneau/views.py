...
def abc_creneau_view(request):
    ...
    try:
        ...
        print(
            f"Creneaux for this AbonnementClient (ID: {abonnement_client.id}): {creneaux}"
        )
...
class CreateCreneau(PermissionRequiredMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        posted_data = "\n.".join(
            f"{key} {value}" for key, value in request.POST.items()
        )
        print("POSTED DATA =========\n", posted_data, "\n==========")
        if form.is_valid():
            form.save()
            ...
            return HttpResponse(
                ...
                headers={
                    "HX-Trigger": json.dumps(
                        {"closeModal": "kt_modal", "refresh_calendar": None}
                    )
                },
            )
        else:
...
class CalenderView(PermissionRequiredMixin, FilterView):
    ...
    def get_events(self):
        events = self.filterset_class(
            self.request.GET, queryset=Creneau.objects.all()
        ).qs
                ...
                events_list.append(
                    {
                        ...
                        "url": reverse(
                            "creneau:update_creneau", kwargs={"pk": event.pk}
                        ),
                    }
                )
        return events_list
...
class AbonnementsParCreneau(SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        ...
        latest_abonnements = (
            AbonnementClient.objects.filter(creneaux__pk=creneau_pk)
            .values("client")
            .annotate(latest_created_date_time=Max("created_date_time"))
        )
        # Filter original queryset with the annotated max created_date_time
        queryset = (
            AbonnementClient.objects.select_related("client", "type_abonnement")
            .filter(
                creneaux__pk=creneau_pk,
                end_date__gte=date.today(),
                created_date_time__in=[
                    item["latest_created_date_time"] for item in latest_abonnements
                ],
            )
    ...
    def get_table_kwargs(self):
        return {"creneau_pk": self.kwargs.get("pk")}
