...
class RangeManager(models.Manager):
    ...
    def get_creneau(self):
        ...
        creneaux_shifted = []
        now = datetime.now().time()
        creneaux = self.get_creneaux_of_day() or []
        for i in creneaux:
            # print('the i', i)
            try:
                start_hour_1 = i.hour_start.replace(
                    hour=i.hour_start.hour, minute=i.hour_start.minute - 20
                )
            except:
                try:
                    start_hour_1 = i.hour_start.replace(
                        hour=i.hour_start.hour - 1, minute=60 + i.hour_start.minute - 20
                    )
                except:
                    start_hour_1 = i.hour_start.replace(
                        hour=23, minute=60 + i.hour_start.minute - 20
                    )
            try:
                start_hour_2 = i.hour_start.replace(
                    hour=i.hour_start.hour, minute=i.hour_start.minute + 20
                )
            except:
                try:
                    start_hour_2 = i.hour_start.replace(
                        hour=i.hour_start.hour + 1, minute=i.hour_start.minute + 20 - 60
                    )
                except:
                    start_hour_2 = i.hour_start.replace(
                        hour=0, minute=i.hour_start.minute + 20 - 60
                    )
    ...
    def get_similar_creneaux(self, creneau_id):
        ...
        try:
            start_hour_1 = st_hour.replace(
                hour=st_hour.hour, minute=st_hour.minute - 20
            )
        except:
            try:
                start_hour_1 = st_hour.replace(
                    hour=st_hour.hour - 1, minute=60 + st_hour.minute - 20
                )
            except:
                start_hour_1 = st_hour.replace(hour=23, minute=60 + st_hour.minute - 20)
        try:
            start_hour_2 = st_hour.replace(
                hour=st_hour.hour, minute=st_hour.minute + 20
            )
        except:
            try:
                start_hour_2 = st_hour.replace(
                    hour=st_hour.hour + 1, minute=st_hour.minute + 20 - 60
                )
            except:
                start_hour_2 = st_hour.replace(hour=0, minute=st_hour.minute + 20 - 60)
        tous_creneaux = Creneau.objects.filter(
            hour_start__range=[start_hour_1, start_hour_2]
        )
...
class Creneau(models.Model):
    ...
    day = models.CharField(
        choices=DAYS_CHOICES, max_length=2, default="DI", verbose_name="Jour"
    )
    name = models.CharField(
        verbose_name="nom du creneau", max_length=100, blank=True, null=True
    )
