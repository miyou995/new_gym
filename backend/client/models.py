...
class Client(models.Model):
    ...
    last_name = models.CharField(
        max_length=50, verbose_name="Nom", blank=True, null=True
    )
    ...
    phone = models.CharField(
        max_length=22, verbose_name="Téléphone", blank=True, null=True
    )
    email = models.CharField(
        max_length=50, verbose_name="E-mail", blank=True, null=True
    )
    nationality = models.CharField(
        max_length=50, verbose_name="Nationalité", blank=True, null=True
    )
    ...
    dette_assurance = models.DecimalField(
        max_digits=10, decimal_places=0, blank=True, null=True, default=0
    )
    ...
    def remove_duplicate(self):
        ...
        logger.warning(
            "remove_duplicated presences ================> {}".format(str(self.carte))
        )
        if presences:
    ...
    def auto_presence(self, door_ip=None):
        ...
        client = self
        status_sortie = self.init_output()
        if status_sortie:
            if status_sortie != "skip":
                response["status"] = "sortie"
            return response

        print("You should proceeed to entree")
        # client.has_permission()
        creneaux = (
            Creneau.range.get_creneaux_of_day()
            .filter(abonnements__client=client)
            .distinct()
        )
        ...
        if len(creneaux):
            dur_ref_time_format = abs(
                datetime.strptime(str(creneaux[0].hour_start), FTM)
                - datetime.strptime(current_time, FTM)
            )
            ...
            for cr in creneaux:
                ...
                temps = abs(
                    datetime.strptime(start, FTM) - datetime.strptime(current_time, FTM)
                )
                duree_seconde = timedelta.total_seconds(temps)
                if dur_ref > duree_seconde:
                    dur_ref = duree_seconde
                    cren_ref = cr
            abon_list = AbonnementClient.objects.filter(
                client=client, end_date__gte=date.today()
            ).order_by("-presence_quantity")
            if not abon_list:
                # raise serializers.ValidationError("l'adherant n'est pas inscrit aujourd'hui")
                print(
                    "*******l'adherant n'est pas inscrit aujourd'hui ou n'a pas d'abonnement**********"
                )
                response["status"] = "not_today"
                return response
            ...
            if not abonnement:
                abonnement = (
                    abon_list.first()
                    if abon_list.first().is_valid()
                    else abon_list.last()
                )

            if abonnement.is_valid() and abonnement.is_time_volume():
                print(
                    "IM HEEERE LOG ABONNEMENT==== TIME VOLUUUPME",
                    abonnement.is_time_volume(),
                )
                Presence.objects.create(
                    abc=abonnement, creneau=cren_ref, hour_entree=current_time
                )
                ...
                response["abc"] = abonnement
                response["status"] = "fin_abonnement"
                return response
        else:
            print(
                "*******l'adherant n'est pas inscrit aujourd'hui pas de abc **********"
            )
            response["status"] = "not_today"
            return response
    ...
    def init_output(self, exit_hour=None):
        ...
        with transaction.atomic():
            ...
            if not presence:
                """ ecart + de 10 seconde"""
                print(
                    "No presence found or presence time difference is more than 10 seconds"
                )
                return False
            current_time = datetime.now().strftime("%H:%M:%S")
            if exit_hour:
    ...
    def manuelle_presence(self, date, heur_sortie, door_ip=None):
        ...
        creneaux = Creneau.objects.filter(
            abonnements__client=client, day=date
        ).distinct()
        logger.warning("LOGLes creneaux du Today client=====-{}".format(str(creneaux)))
        if len(creneaux):
            abonnement = creneaux
            ...
            print(
                "ABONNEMENT is valid<<<<<<<<<<>>>>>>>>>>>>>>>>", abonnement.is_valid()
            )
            print(
                "ABONNEMENT presence qnt<<<<<<<<<<>>>>>>>>>>>>>>>>",
            ...
            if abonnement.is_valid() and abonnement.is_time_volume():
                ...
                logger.warning(
                    "LOG abonnement.presence_quantity=====> {}".format(
                        str(abonnement.presence_quantity)
                    )
                )
                logger.warning(
                    "LOG ABONNEMENT TYPE=====-{}".format(
                        str(abonnement.type_abonnement.type_of)
                    )
                )
    ...
    def get_access_permission(self, door_ip=None):
        ...
        logger.warning(
            "Nombre de presence en cours=> {}".format(str(presences.count()))
        )
        current_time = datetime.now().strftime("%H:%M:%S")
        ...
        abonnement_client = abonnements_actives.filter(
            client=self, type_abonnement__salles=salle
        ).first()
        creneaux = Creneau.range.get_creneaux_of_day().filter(
            abonnements=abonnement_client
        )
        ...
        logger.warning(
            "Le TYPE DABONNEMENT CRENEAUX ====={}".format(str(abonnement_client))
        )
        ...
        if abonnement_client.is_time_volume() and abonnement_client.is_valid():
            ...
            # with transaction.atomic():
            Presence.objects.create(
                abc=abonnement_client, creneau=cren_ref, hour_entree=current_time
            )
            self.save()
            ...
            if creneaux.count() > 1:
                dur_ref_time_format = abs(
                    datetime.strptime(str(creneaux[0].hour_start), FTM)
                    - datetime.strptime(current_time, FTM)
                )  # nous avons besoin d'un crenaux Reference pour le comparé au autres
                dur_ref = timedelta.total_seconds(dur_ref_time_format)
                for cr in creneaux:
                    start = str(cr.hour_start)
                    # print('heure de début', start)
                    temps = abs(
                        datetime.strptime(start, FTM)
                        - datetime.strptime(current_time, FTM)
                    )
                    duree_seconde = timedelta.total_seconds(temps)
                    if dur_ref > duree_seconde:
            ...
            logger.warning(
                "WHATS THE CASE-- {}".format(str(abonnement_client.type_abonnement))
            )
            return False
...
class Coach(models.Model):
    ...
    first_name = models.CharField(max_length=50, verbose_name="Prénom")
    civility = models.CharField(
        choices=CIVILITY_CHOICES, max_length=3, default="MME", verbose_name="Civilité"
    )
    adress = models.CharField(
        max_length=200, verbose_name="Adresse", blank=True, null=True
    )
    ...
    state = models.CharField(
        choices=STATE_CHOICES, max_length=3, verbose_name="Etat", default="A"
    )
    note = models.TextField(blank=True, null=True)
    ...
    pay_per_hour = models.IntegerField(
        blank=True, null=True, default=1, verbose_name="Salaire par heure"
    )
    pay_per_hour = models.IntegerField(
        blank=True, null=True, default=1, verbose_name="Salaire par heure"
    )
    ...
    def enter_sotrie_coach(self):
        pending_presence = PresenceCoach.objects.filter(
            coach=self, hour_sortie__isnull=True
        )
        print("in_salle ------------------------", pending_presence)

        if not pending_presence:
            pending_presence = PresenceCoach.objects.get_or_create(
                coach=self, hour_entree=timezone.now().time()
            )
        else:
...
class Personnel(models.Model):
    ...
    function = models.CharField(
        max_length=200, verbose_name="Fonction", blank=True, null=True
    )
    phone = models.CharField(
        max_length=22, verbose_name="Téléphone", blank=True, null=True
    )
    ...
    date_added = models.DateTimeField(
        auto_now_add=True, verbose_name="Date de recrutement"
    )
    created = models.DateTimeField(verbose_name="Date de Création", auto_now_add=True)
    updated = models.DateTimeField(
        verbose_name="Date de dernière mise à jour", auto_now=True
