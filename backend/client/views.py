...
from .tables import (
    AbonnementClientHTMxTable,
    ClientHTMxTable,
    CoachDetailHTMxTable,
    CoachHTMxTable,
    PaiementHTMxTable,
    PersonnelHTMxTable,
    PresenceClientHTMxTable,
    PresenceCoachHTMxTable,
    VirementsHTMxTable,
)
...
def ClientCreateView(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponse(
                ...
                headers={
                    "HX-Trigger": json.dumps(
                        {"closeModal": "kt_modal", "refresh_table": None}
                    )
                },
            )
        else:
            ...
            return render(
                request, template_name="snippets/_client_form.html", context=context
            )
    context["form"] = form
...
class ClientDeleteView(PermissionRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        ...
        if abc:
            ...
            messages.error(
                self.request,
                "vous ne pouvez pas supprimer un client avec un abonnement",
            )
...
def CoachCreateView(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request, template_name="snippets/_coach_form.html", context=context
            )
    context["form"] = form
    return render(request, template_name=template_name, context=context)
...
def PersonnelCreateView(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request, template_name="snippets/_personnels_form.html", context=context
            )
    context["form"] = form
    return render(request, template_name=template_name, context=context)
...
class ClientDetailView(SingleTableMixin, FilterView):
    ...
    def get_table_kwargs(self):
        return {"abonnement_client_pk": self.kwargs.get("pk")}
...
class PresenceClientDetail(SingleTableMixin, FilterView):
    ...
    def get_queryset(self):
        queryset = Presence.objects.order_by("-created").select_related(
            "abc", "abc__client", "creneau", "creneau__activity"
        )
        abonnement_client_pk = self.kwargs.get("pk")
...
class CoachDetail(SingleTableMixin, FilterView):
    ...
    def get_table_kwargs(self):
        return {"coach_pk": self.kwargs.get("pk")}
