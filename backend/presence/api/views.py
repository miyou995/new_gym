...
from .models import Presence, PresenceCoach  # pyright: ignore[reportMissingImports]
from .serializers import (
    PresenceAutoSerialiser,
    PresenceClientSerialiser,
    PresenceCoachSerializer,
    PresenceEditSerialiser,
    PresenceHistorySerialiser,
    PresenceManualEditSerialiser,
    PresenceNotificationSerialiser,
    PresencePostSerialiser,
    PresenceSerialiser,
)
...
class PresencePostAPIView(generics.CreateAPIView):
    ...
    extra_perms_map = {"POST": ["presence.add_presence"]}
...
class PresenceHistoryListAPIView(generics.ListAPIView):
    ...
    permission_classes = (IsAdminUser, BaseModelPerm)
    extra_perms_map = {"GET": ["presence.view_presence"]}
...
class NotifyPresenceListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Presence.objects.filter(
            updated__gte=three_seconds_ago, hour_sortie__isnull=False
        )
        # permission_classes = (IsAuthenticated,)
        return queryset
...
class PresenceListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Presence.objects.select_related(
            ...
            "abc",
            "abc__client",
            "abc__type_abonnement",
            "creneau__activity__salle",
            "creneau__planning",
        ).annotate(total_dette=Sum("abc__client__abonnement_client__reste"))
        ...
        try:
            ...
            queryset = queryset.filter(
                ...
                creneau__hour_start__range=[start_time, end_time],
            )
...
class PresenceCoachCreateAPI(generics.CreateAPIView):
    ...
    permission_classes = (IsAdminUser, BaseModelPerm)
    extra_perms_map = {"POST": ["presence.add_presencecoach"]}
...
class PresenceCoachListAPI(generics.ListAPIView):
    ...
    serializer_class = PresenceCoachSerializer
    permission_classes = (IsAdminUser, BaseModelPerm)
    extra_perms_map = {"GET": ["presence.view_presencecoach"]}
    ...
    def get_queryset(self):
        ...
        presences = (
            Presence.objects.filter(
                abc__client_id=client, date__range=[start_date, end_date]
            )
            .select_related(
                "creneau",
                "abc",
...
class PresenceClientIsInAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # print('client', client)
        presences = Presence.objects.filter(hour_sortie__isnull=True).select_related(
            "creneau",
            "abc",
            "abc__client",
            "abc__type_abonnement",
