...
const toggleSuspension = () => {
    // Placeholder for your URL, replace it with the actual one
    const url = `${process.env.REACT_APP_API_URL}/rest-api/abc-toggle-suspension/${abonId}/` ;
	

    api.post(url, {/* Your request payload if any */})
      .then(response => {
        // Handle the response
        // Update the suspension status based on the response or the next expected state
			setIsSuspended(!isSuspended);
			notifySuccess("Abonnement Bloqué/Débloqué")
      })
      .catch(error => {
        // Handle any errors here
        console.error('There was an error!', error);
		notifyError("Erreur lors du bloquage / débloquage, contacter le support")
...
useEffect(() => {
	if (show == true) {
		...
		setIsSuspended(abonnementData['isLocked'])
		setAbcData(abonnementData['abonnementData'])
		...
		<Tab.Container defaultActiveKey="monthly">
			<div className="row justify-content-center">
					...
					<div className="media profile-bx">
						<div className="media-body align-items-center">
							...
							<div className="card-header border-0 d-xl-flex d-lg-block d-md-flex d-sm-flex d-block">
								...
								{ isSuspended &&
									<h4 className="fs-20 text-black">Suspendu le: <a className="item text-primary">{abcData.blocking_date} </a> </h4>
								}
								</div>
								...
								<form>
									<div className="row">
										<div className="form-group col-md-4">
											...
											<div className="form-group col-md-4">
											<label className="text-black">Reste seances / minutes</label>
											<input type="number" value={presences} className="form-control" onChange={e => setPresences(e.target.value)}/>
											</div>
											<div className="form-group col-md-4">
											<label className="text-black">Reste (DA)</label>
				...
				<div className="col-12">
					...
					<Button 
						variant={isSuspended ? 'success' : 'info'}
						className='m-2'
						onClick={toggleSuspension}
						> 
						{isSuspended ? 'Débloqué' : 'Suspendre'}
					</Button>
				</div>
			</div>
		</Tab.Container>
